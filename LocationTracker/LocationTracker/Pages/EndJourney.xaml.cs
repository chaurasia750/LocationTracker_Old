	...
	public partial class EndJourney : ContentPage
	{
        ...
        private async void ConfirmButton_Clicked(object sender, EventArgs e)
        {
            ...
            if (current == NetworkAccess.Internet)
            {
                ...
                LoadingIndicatorText.Text = "Ending Journey";
                try
                {
                    ...
                    if (location != null)
                    {
                        ...
                        try
                        {
                            ...
                            if (placemark != null)
                            {
                                ...
                                {
                                    string url = DifferentUrls.EndJourneyUrl;
                                    HttpContent q1 = new FormUrlEncodedContent(new List<KeyValuePair<string, string>>() { new KeyValuePair<string, string>("Username", Application.Current.Properties["Username"].ToString()), new KeyValuePair<string, string>("Password", Application.Current.Properties["Password"].ToString()), new KeyValuePair<string, string>("Addresss", PlaceDetails), new KeyValuePair<string, string>("Timestamp", statime), new KeyValuePair<string, string>("JID", Application.Current.Properties["JID"].ToString()), new KeyValuePair<string, string>("Remarks", remar), new KeyValuePair<string, string>("UserPlace", plac) });
                                            ...
                                            if (response.IsSuccessStatusCode)
                                            {
                                                ...
                                                else
                                                {
                                                    Device.BeginInvokeOnMainThread(async () =>
                                                    {
                                                        ...
                                                        await Application.Current.SavePropertiesAsync();
                                                        var Message = $"Journey completed at {statime}!!";
